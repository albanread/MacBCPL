...
- **[the_new_bcpl_programming_language.md](the_new_bcpl_programming_language.md)** — the language manual: a K&R-style tutorial and reference for New BCPL (with Cocoa). Part I covers the lexis, types, control flow, functions, pointers/vectors, lists, classes, and the memory model; Part II ("Cocoa") opens with strings as `NSString` objects. Grammar and standard-library appendices.
